...
class RoomService {
  ...
  async updateRoom(roomId, userId, data) {
    ...
    /*
     * Populate separately.
     * Do not chain .populate() on the result of
     * document.populate() because the installed Mongoose
     * version returns a Promise from populate().
     */
  ...
  async inviteMember(roomId, userId, { email, role }) {
    ...
    if (
      !requester ||
      !['owner', 'editor'].includes(requester.role)
    ) {
      throw new AppError('Permission denied', 403);
    }
  ...
  async kickMember(roomId, ownerId, targetUserId) {
    ...
    if (
      room.owner.toString() !== ownerId.toString()
    ) {
      throw new AppError(
  ...
  async transferOwnership(
    ...
    const newOwnerMember = room.members.find(
    ...
    const currentOwnerMember = room.members.find(
      (m) =>
        m.user.toString() ===
        currentOwnerId.toString()
    );
  ...
  async createSession(roomId, userId) {
    ...
    if (
      ...
      throw new AppError(
        'Only the workspace owner can start session recording',
        403
      );
    }
  ...
  async endSession(
    ...
    if (
      ...
      throw new AppError(
        'Only the workspace owner can stop session recording',
        403
      );
    }
  ...
  async leaveRoom(roomId, userId) {
    ...
    if (
      ...
      throw new AppError(
        'Owner cannot leave without transferring ownership or deleting the workspace',
        400
      );
    }
